      ...
      <section className="pb-16 min-h-[400px]">
        {isLoading ? (
          ...
          facilities.map((facility, i) => {
            ...
            if (facility.type === "accommodation") {
              typeLabel = "Guest House";
              const roomCount = facility.rooms?.length || 0;
              previewInfo = `${roomCount} Room Type${roomCount !== 1 ? 's' : ''} Available`;
            } else if (facility.type === "event") {
              typeLabel = "Event Space";
              if (facility.pricing && facility.pricing.length > 0) {
                previewInfo = `Starting from ₹${facility.pricing[0].amount}`;
              } else {
                previewInfo = "Flexible Pricing";
              }
            }
